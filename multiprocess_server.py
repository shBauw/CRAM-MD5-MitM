...
def send(msg):
    # Format print for multi-processing
    msg = msg + '\r\n'
...
def server(s, inbox, c, addr):
    ...
    num = info.counter
    pid = os.getpid()
    info.multi = "[" + str(pid) + "][" + str(num).zfill(2) + "]"
    ...
    while True:
        ...
        # Connection lost
        if not data:
            print(info.multi + "S: Connection lost\r\n", flush=True, end='')
            break
        else:
            data = data.decode("ascii")
            print(info.multi + "C: %s" % (data), end='', flush=True)
...
def main():
    ...
    # Multiprocessing: Listen, fork to run rest of commands after connection created.
    s.listen()
    while True:
        c, addr = s.accept()
        info.counter += 1
